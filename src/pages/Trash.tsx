    ...
    <div className="mx-auto max-w-3xl px-5 py-6 sm:px-8">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-zinc-900">Trash</h1>
            <p className="mt-0.5 text-xs text-zinc-400">Deleted transactions are kept here until restored or permanently removed.</p>
          </div>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              {activeSelectedIds.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100 disabled:opacity-50"
                >
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 transition hover:bg-red-100"
              >
      ...
      {deletedExpenses.length === 0 ? (
        <div className="rounded-2xl border border-zinc-200 bg-white py-16 text-center shadow-xs">
          ...
          <p className="mt-3 text-sm font-semibold text-zinc-700">Trash is empty</p>
          <p className="mt-1 text-xs text-zinc-400">Deleted transactions will appear here.</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-zinc-200/90 bg-white shadow-xs">
          {deletedExpenses.map((expense) => (
            <div key={expense.id} className={`flex items-center justify-between gap-4 border-b border-zinc-100 px-4 py-4 last:border-b-0 ${
              selectedIds.includes(expense.id) ? "bg-emerald-50/50" : ""
            }`}>
              <div className="flex min-w-0 items-center gap-3">
                ...
                <p className="truncate text-sm font-semibold text-zinc-900">{expense.item}</p>
              ...
              <button
                ...
                className="flex shrink-0 items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100 disabled:opacity-50"
              >
                <ArchiveRestore className="h-3.5 w-3.5" />
